...
const heroResultIntro = document.querySelector(".hero-result-intro");
const heroResultHeader = document.querySelector(".hero-result-header");
const heroResultContent = document.querySelector(".hero-result-content");
...
const heroResultWelcome = document.querySelector(".hero-result-welcome");
...
function updateBMI(input) {
  ...
  //if opposite input is valid, update BMI
  if (input) {
    ...
    heroResultContent.style.width = `50%`;

    //Use BMI formula corresponding to the selected. Limited to 4 characters and 1 decimal.
    ...
    //Infinity result is outputted as a string, after being sliced to 4 chars
    if (bmiResult.textContent === "Infi") bmiResult.textContent = "0.0";

    const bmiInterpreted = Number(bmiResult.textContent);

    //change the description according to the BMI range
    if (bmiInterpreted < 18.5) {
      weightResult.textContent = "underweight. Your ideal weight is ";
    ...
    if (bmiInterpreted >= 18.5 && bmiInterpreted <= 24.9) {
      weightResult.textContent = "a healthy weight. Your ideal weight is ";
    ...
    if (bmiInterpreted >= 25 && bmiInterpreted < 29.9) {
      weightResult.textContent = "overweight. Your ideal weight is ";
    ...
    if (bmiInterpreted >= 30) {
      weightResult.textContent = "obese. Your ideal weight is ";
